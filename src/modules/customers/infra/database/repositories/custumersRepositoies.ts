...
import { ICustomersRepository, Pagination } from '../../../domains/repositories/ICustumerRepository';
...
export default class customersRepository implements ICustomersRepository {
  ...
  constructor() { this.ormRepository = AppDataSource.getRepository(Customer); }
  ...
  async findAndCount({take,skip,}: Pagination): Promise<[ICustomer[], number]> {
    const [customer, total] = await this.ormRepository.findAndCount({take,skip})
